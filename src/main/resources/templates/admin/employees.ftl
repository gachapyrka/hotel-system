...
<@common.page title="Управление аккаунтами">
        ...
        <div class="row g-2 align-items-center justify-content-between">
            <div class="col-5">
                <div class="row g-2 align-items-center justify-content-start">
                    <div class="col-auto">
                        <a class="btn btn-secondary" href="/admin/users">Клиенты</a>
                    </div>
                    <div class="col-auto">
                        <a class="btn btn-primary disabled" href="/admin/employees">Сотрудники</a>
                    </div>
                </div>
            </div>
            <div class="col-7">
    ...
    <#if employees??>
        ...
        <div class="container mt-3">
            <table class="table table-striped table-hover table-sm">
                ...
                <#list employees as e>
                    <tr>
                        <th class="col-1" scope="row">${e_index + 1}.</th>
                        <td class="col-2">${e.account.username}</td>
                        <td class="col-2">${e.credentials}</td>
                        <td class="col-2">${e.telephone}</td>
                        <td class="col-3">${e.hotel.name}</td>
                        <td class="col-1">
                            <form method="get" action="/admin/message/${e.account.id}">
                                <input type="submit" class="btn btn-sm btn-primary" value="Сообщение">
                                <input type="hidden" name="_csrf" value="${_csrf.token}" />
                            </form>
                        </td>
                        <td class="col-1">
                            <form method="post" action="/admin/accounts/${e.account.id}">
                                <#if e.account.isActive()>
        ...
        <form method="post" id="newsletterForm" action="/admin/employees/newsletter">
    ...
    <script>
        ...
        function onSearch() {
            ...
            var request = search == null? "//localhost:8080/admin/employees" :
                "//localhost:8080/admin/employees?search="+search;
